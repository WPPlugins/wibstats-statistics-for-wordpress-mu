...
if ( function_exists( "add_action" ) ) {
	...
	// for activation
	add_action( 'wibstats/wibstats.php', 'wibstats_activate' );
...
function wibstats_add_admin() {
	...
	add_submenu_page( "options-general.php", "Blog statistics settings", "Blog statistics settings", 1, "wibstats-options", "wibstats_options" ); 
...
function wibstats_siteadmin_options() {
	...
	if ( $wibstats_mu && is_site_admin() ) {
		if ( isset( $_GET["view"] ) && $_GET["view"] == "siteadmin" ) {
			...
			<form action="admin.php?page=wibstats-siteadmin" method="post">
			...
			echo '
			<p><a href="admin.php?page=wibstats-siteadmin">View site admin options</a></p>
			';
...
function wibstats_options() {
	...
	$wibstats_gmap_key = get_option( "wibstats_gmap_key" );
...
function wibstats()
{
	...
	if ( $current_user->ID == 0 || !$disable_logged_in_user )
	{
		if ( $wibstats_mu )
		{
			...
			$path = get_option( "siteurl" ) . "/wp-content/plugins/wibstats/";
		}
		$js = '
...
function wibstats_country_icon( $country, $code )
{
	if ( strlen( $code ) != 2 )
	{
		...
		if ( $wibstats_mu )
		{
			...
			return '<img src="' . get_option( "siteurl" ) . '/wp-content/plugins/wibstats/wibstats-includes/flags/' . $code . '.png" alt="' . $country . '" /> ';
		}
	}
}
